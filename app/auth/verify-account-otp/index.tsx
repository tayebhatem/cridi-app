...
import { sendOtp, sendVerificationCode, verifyAccountOtp } from '@/libs/appwrite';
        ...
        <SafeAreaView className='bg-white dark:bg-dark-500 p-6 h-full space-y-4'>
            ...
            <View>
                ...
                <Text className="text-neutral-400 font-kufi leading-6">
                    {language?.id === 'en' ? 'Please enter the OTP sent to your email '+account?.email : language?.id === 'fr' ? 'Veuillez entrer le code de vérification envoyé à votre email '+account?.email : 'يرجى إدخال رمز التحقق المرسل إلى بريدك الإلكتروني '+account?.email}
                </Text>
