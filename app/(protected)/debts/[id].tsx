        ...
        <View className="flex flex-row items-center">
          {filterDateData.map((item) => (
            <TouchableOpacity
              ...
              key={item.id}
              activeOpacity={0.8}
              className={` px-6 rounded-full shadow-primary-500 shadow-md ${
                item.id === range && "bg-white dark:bg-dark-400"
              }`}
            >
              <Text
                className={`font-kufi-medium  ${
                  item.id === range
          ...
          <CardLayout elevationAndroid={1}>
            <FlatList
              ...
              contentContainerStyle={{
                marginVertical: debts && debts?.length === 0 ? "auto" : 0,
              }}
              refreshControl={
