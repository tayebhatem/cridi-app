...
if(!language) return <Loader/>
  return (
    ...
    <Text className="text-2xl font-kufi-semi-bold">
        {language?.id === 'en' ? 'Complete supplier account' : language?.id === 'fr' ? 'Compléter le compte fournisseur' : 'أكمل حساب المورد'}
    </Text>
    <Text className="text-neutral-400 font-kufi leading-6">
      ...
      <View>
      <Button onChange={onSave} title={translations.saveButton[language.id]} />
      </View>
    </SafeAreaView>
  );
};
